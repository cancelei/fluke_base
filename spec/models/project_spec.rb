...
RSpec.describe Project, type: :model do
  ...
  describe "instance methods" do
    ...
    describe "#recent_github_logs" do
      it "orders by commit_date desc and respects limit" do
        older = create(:github_log, project: project, user: user, commit_date: 3.days.ago)
        middle = create(:github_log, project: project, user: user, commit_date: 2.days.ago)
        newest = create(:github_log, project: project, user: user, commit_date: 1.day.ago)
    ...
    describe "GitHub integration helpers" do
      it "returns available_branches via service" do
        b1 = create(:github_branch, project: project, branch_name: 'feature/a')
  ...
  describe "github_contributions filters" do
    let!(:branch_main) { create(:github_branch, project: project, user: user, branch_name: 'main') }
    let!(:branch_feat) { create(:github_branch, project: project, user: user, branch_name: 'feature/x') }

    def create_log(u:, branch:, added: 1, removed: 0, name: nil)
      log = create(:github_log, :with_user, project: project, user: u, unregistered_user_name: name, lines_added: added, lines_removed: removed, commit_date: Time.current)
    ...
    it "filters by branch, agreement_only, and user_name" do
      ...
      bot_log = create(:github_log, :unregistered, project: project, unregistered_user_name: 'bot', lines_added: 10, lines_removed: 0, commit_date: Time.current)
      ...
      mentor = create(:user)
      create(:agreement, :with_participants, :mentorship, project: project, initiator: user, other_party: mentor, status: Agreement::ACCEPTED)
  ...
  describe "visibility helpers" do
    it "field_public? respects public_fields" do
      p = create(:project, user: user, public_fields: [ 'name', 'description' ])
    ...
    it "visible_to_user? is true for owner and for public field, false otherwise" do
      p = create(:project, user: user, public_fields: [ 'name' ])
      stranger = create(:user)
    ...
    it "get_field_value returns value only if visible" do
      p = create(:project, user: user, name: 'Alpha', description: 'Hidden', public_fields: [ 'name' ])
      stranger = create(:user)
