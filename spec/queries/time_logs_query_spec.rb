...
RSpec.describe TimeLogsQuery do
  ...
  before do
    create(:agreement, :with_participants, :mentorship, project:, initiator: owner, other_party: participant, status: Agreement::ACCEPTED)
  end

  describe '#milestones_for_project' do
    it 'returns all milestones for owner' do
      m1 = create(:milestone, project:)
    ...
    it 'returns only agreement milestones for participant' do
      m_allowed = create(:milestone, project:)
      m_other = create(:milestone, project:)
  ...
  describe '#time_logs_for_project' do
    it 'filters by selected date and orders desc' do
      ...
      create(:time_log, project:, milestone: m, user: owner, started_at: Time.zone.parse('2024-01-01 10:00'))
      today_log = create(
        :time_log,
        project:,
        milestone: m,
  ...
  describe '#filtered_time_logs' do
    it 'applies user filter and date' do
      ...
      started_at = 2.hours.ago
      selected_date = started_at.to_date
      a = create(:time_log, project:, milestone: m, user: owner, started_at:, ended_at: started_at + 1.hour)
      b = create(:time_log, project:, milestone: m, user: participant, started_at:, ended_at: started_at + 1.hour)
  ...
  describe '#milestones_pending_confirmation' do
    it 'returns in_progress milestones with completed time_logs' do
      m = create(:milestone, project:, status: Milestone::IN_PROGRESS)
      create(:time_log, project:, milestone: m, user: owner, status: 'completed')
  ...
  describe '#time_logs_completed' do
    it 'returns completed milestone logs on selected date' do
      m = create(:milestone, project:, status: Milestone::COMPLETED)
      create(:time_log, project:, milestone: m, user: owner, status: 'completed', started_at: Time.zone.now, ended_at: Time.zone.now + 1.hour)
  ...
  describe '#users_for_filter' do
    it 'returns distinct users who logged time on given milestones' do
      m = create(:milestone, project:)
      create(:time_log, project:, milestone: m, user: owner)
  ...
  describe '#manual_time_logs' do
    it 'returns manual time logs for current user by default' do
      manual = create(:time_log, project:, user: owner, milestone: nil, manual_entry: true)
